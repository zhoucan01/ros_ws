...
int planarVoxelLowObsCount[kPlanarVoxelNum] = {0};
...
int main(int argc, char **argv) {
  ...
  nh->declare_parameter<int>("minLowObstaclePointNum", minLowObstaclePointNum);
  ...
  nh->get_parameter("minLowObstaclePointNum", minLowObstaclePointNum);
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      // Count occupied points below the vehicle height for each planar voxel.
      // Hanging obstacles such as crossbars should not become obstacles if the
      // free space below the robot height is empty.
      for (int i = 0; i < terrainCloudSize; i++) {
        point = terrainCloud->points[i];
        if (point.z - vehicleZ > minRelZ && point.z - vehicleZ < maxRelZ) {
          int indX =
          ...
          if (indX >= 0 && indX < planarVoxelWidth && indY >= 0 &&
            ...
            if (planarVoxelDyObs[planarVoxelWidth * indX + indY] <
              ...
              if (considerDrop)
                disZ = fabs(disZ);
              if (disZ >= 0 && disZ < vehicleHeight &&
                  planarVoxelLowObsCount[planarVoxelWidth * indX + indY] >=
                      minLowObstaclePointNum) {
