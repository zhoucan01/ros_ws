...
mask 图使用的是它自己的 `origin + resolution + frame_id` 定义出的坐标系。最方便的做法是让它和主地图完全同尺寸、同分辨率、同 origin，并把 `frame_id` 设成 `map`。如果你的地图是从某个建图起点建立的，那么这张 mask 图本质上也是“基于建图起点/地图原点”的图。
...
上述示例表示：`IntensityVoxelLayer` 会加载一张与主地图对齐的白名单 mask 图。你只需要在这张图上把“允许直接通过、不要推进 costmap 的区域”涂出来即可。这个功能适合用于处理已知固定场地中的限高杆、悬空横梁等误检区域。

**怎么做最方便：**

1. 复制一份主地图对应的 `pgm`，例如 `rmuc_2025.pgm`。
...
- 白色（或高灰度）区域：白名单通行区，点落入后不会推进 costmap。
- 黑色（或低灰度）区域：正常区域，按原逻辑处理。

建议直接复用主地图的尺寸和原点，这样你只需要对着实际地图在图上画，不需要再手动算矩形坐标。
